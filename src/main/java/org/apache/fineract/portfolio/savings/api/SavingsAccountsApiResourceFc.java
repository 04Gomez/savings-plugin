...
public class SavingsAccountsApiResourceFc {

    private final SavingsAccountReadPlatformService savingsAccountReadPlatformService;
    private final SavingsAccountReadPlatformServiceImplFc savingsAccountReadPlatformServiceFc;
    private final PlatformSecurityContext context;
    private final DefaultToApiJsonSerializer<SavingsAccountData> toApiJsonSerializer;
    private final DefaultToApiJsonSerializer<SavingsAccountDataFc> toApiJsonSerializerFc;
    ...
    private String retrieveSavingAccount(Long accountId, String externalId, boolean staffInSelectedOfficeOnly, String chargeStatus,
        ...
        final SavingsAccountDataFc savingsAccount = savingsAccountReadPlatformServiceFc.retrieveOneFc(accountId);

        final Set<String> mandatoryResponseParameters = new HashSet<>();
        final SavingsAccountDataFc savingsAccountTemplate = populateTemplateAndAssociationsFc(accountId, savingsAccount,
                staffInSelectedOfficeOnly, chargeStatus, uriInfo, mandatoryResponseParameters);

        final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters(),
                mandatoryResponseParameters);
        return toApiJsonSerializerFc.serialize(settings, savingsAccountTemplate,
    ...
    private SavingsAccountDataFc populateTemplateAndAssociationsFc(final Long accountId, final SavingsAccountDataFc savingsAccount,
            final boolean staffInSelectedOfficeOnly, final String chargeStatus, final UriInfo uriInfo,
            final Set<String> mandatoryResponseParameters) {

        Collection<SavingsAccountTransactionDataFc> transactions = null;
        Collection<SavingsAccountChargeData> charges = null;
        ...
        if (!associationParameters.isEmpty()) {
            ...
            if (associationParameters.contains(SavingsApiConstants.transactions)) {
                mandatoryResponseParameters.add(SavingsApiConstants.transactions);
                final Collection<SavingsAccountTransactionDataFc> currentTransactions = savingsAccountReadPlatformServiceFc
                        .retrieveAllTransactionsFc(accountId, DepositAccountType.SAVINGS_DEPOSIT);
                if (!CollectionUtils.isEmpty(currentTransactions)) {
                    transactions = currentTransactions;
                }
            }
            ...
            if (associationParameters.contains(SavingsApiConstants.charges)) {
                mandatoryResponseParameters.add(SavingsApiConstants.charges);
                final Collection<SavingsAccountChargeData> currentCharges = savingsAccountReadPlatformService
                        .retrieveSavingsAccountCharges(accountId, chargeStatus);
                if (!CollectionUtils.isEmpty(currentCharges)) {
                    charges = currentCharges;
                }
        ...
        SavingsAccountDataFc templateData = null;
        final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
        /*
        if (settings.isTemplate()) {
            templateData = savingsAccountReadPlatformServiceFc.retrieveTemplate(savingsAccount.getClientId(), savingsAccount.getGroupId(),
                    savingsAccount.getSavingsProductId(), staffInSelectedOfficeOnly);
        }*/

        return SavingsAccountDataFc.withTemplateOptionsFc(savingsAccount, templateData, transactions, charges);
    }
}
